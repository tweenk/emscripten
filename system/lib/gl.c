...
void* emscripten_GetProcAddress(const char *name_) {
  ...
  if (0) ;
  // Main list. Regenerate with the following Bash script:
  // grep 'gl.*__sig' src/library_gl.js | sed -e 's/  \([A-Za-z0-9_]*\)__sig.*$/  else if (!strcmp(name, "\1")) return emscripten_\1;/' | grep -v 'ARB\|EXT'
  ...
  else if (!strcmp(name, "glDeleteObject")) return emscripten_glDeleteObject;
  else if (!strcmp(name, "glGetObjectParameteriv")) return emscripten_glGetObjectParameteriv;
  else if (!strcmp(name, "glGetInfoLog")) return emscripten_glGetInfoLog;
  else if (!strcmp(name, "glBindProgram")) return emscripten_glBindProgram;
